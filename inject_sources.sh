...
if [[ $1 ]] && [[ $2 ]] && [[ $3 ]] && [[ $4 ]] && [[ $5 ]] && [[ $6 ]] ; then
    ...
    # base completeness directory
    completeness_dir=$(echo $7 | awk -F"=" '{print $NF}')
else
    echo "Give me: input_map_dir input_sources flux_dir sigma output_dir imageset_name completeness_dir"
    exit 1
fi
...
then
    ...
    singularity exec \
    -B "${output_dir}/flux${SLURM_ARRAY_TASK_ID},$input_map_dir,$output_dir,$completeness_dir/source_pos" \
    "$CONTAINER" \
    aegean \
    --progress \
...
for ((i=1; i<=($nflux); i++ )); do
    ...
    singularity exec \
    -B "${output_dir}/flux${SLURM_ARRAY_TASK_ID},$input_map_dir,$output_dir,$completeness_dir/source_pos/,$MYCODE" \
    "$CONTAINER" \
    ...
    singularity exec \
    -B "${output_dir}/flux${SLURM_ARRAY_TASK_ID},$completeness_dir/source_pos/,${input_map_dir}" \
    "$CONTAINER" \
    AeRes \
